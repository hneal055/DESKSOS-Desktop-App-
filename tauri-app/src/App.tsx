    ...
    <div className="space-y-6">
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => runAction("Flush DNS", () => invoke("flush_dns"))} disabled={loading} className="px-4 py-3 rounded bg-blue-600 hover:bg-blue-700 text-white font-semibold">Flush DNS</button>
          <button onClick={() => runAction("Renew IP", () => invoke("renew_ip"))} disabled={loading} className="px-4 py-3 rounded bg-blue-600 hover:bg-blue-700 text-white font-semibold">Renew IP</button>
          <button onClick={() => runAction("Reset Network", () => invoke("reset_network"))} disabled={loading} className="px-4 py-3 rounded bg-red-600 hover:bg-red-700 text-white font-semibold">Reset Network</button>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => runAction("Restart Spooler", () => invoke("restart_print_spooler"))} disabled={loading} className="px-4 py-3 rounded bg-blue-600 hover:bg-blue-700 text-white font-semibold">Restart Spooler</button>
          <button onClick={() => runAction("Clear Queue", () => invoke("clear_print_queue"))} disabled={loading} className="px-4 py-3 rounded bg-blue-600 hover:bg-blue-700 text-white font-semibold">Clear Queue</button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <button onClick={loadProcesses} disabled={loading} className="mb-4 px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 text-white">Refresh</button>
        <div className="space-y-2">
          {processes.map((p: any, i: number) => (
            <div key={i} className="bg-gray-700 p-3 rounded flex justify-between items-center">
              ...
              <button onClick={async () => { try { await invoke("kill_process", { pid: p.pid }); loadProcesses(); } catch(e) {alert(e);} }} className="px-3 py-1 rounded bg-red-600 hover:bg-red-700 text-white text-sm">Kill</button>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <button onClick={runCommand} disabled={loading} className="px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 text-white font-semibold">{loading ? "Running..." : "Execute"}</button>
  ...
  return (
    <div className="flex h-screen bg-gray-900">
      <div className="w-48 bg-gray-800 p-4 border-r border-gray-700">
        ...
        <div className="space-y-2">
          {modules.map((mod) => (
            <button key={mod.id} onClick={() => setActiveModule(mod.id)} className={`w-full text-left px-3 py-2 rounded transition ${activeModule === mod.id ? "bg-blue-600 text-white" : "text-gray-400 hover:bg-gray-700"}`}>{mod.name}</button>
          ))}
        </div>
      </div>
